...
class Level_three extends Levels {
  List<List<File?>>? wavs;
  var logger = Logger();
        ...
        if (i < length) {
          delayTimer = Timer(Duration(milliseconds: second), () async {
            if (p1.state != PlayerState.disposed) {
              await p1.play(DeviceFileSource(list_per_reading!.elementAt(i)!.path));
              i++;
            }
          });
        } else {
          if (count < 2) {
            ...
            i=1;
            count++;
            delayTimer=Timer(Duration(milliseconds: 6000),() async {
              if (p1.state != PlayerState.disposed) {
            ...
            delayTimer = Timer(Duration(milliseconds: 10000), () {
              function();
            });
          }
        }
